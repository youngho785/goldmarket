    ...
    <>
      ...
      <P_Container>
        ...
        <P_Section>
          ...
          <ul>
            ...
            <li>
              ...
              정규화한 뒤 SHA-256으로 일방향 해시 처리한 가명 식별값,
              혜택 종류·수령 여부·기록 시각, 미사용 적립 순금 잔액, 승계 상태·시각,
              기존 UID 원문을 저장하지 않는 일방향 계정 연속성 식별값
            </li>
            <li>
        ...
        <P_Section>
          ...
          <p>
            진행 중인 예약·교환이 있는 경우 해당 건을 완료하거나 정상 취소한
            후 회원 탈퇴를 진행할 수 있습니다. 탈퇴가 완료되면 Firebase 인증
            계정, 프로필·연락처, 푸시 토큰, 프로필 이미지 등 계정에 연결된
            개인정보를 삭제하며, 회원탈퇴만으로 진행 중인 예약·교환을 자동
            취소하지 않습니다.
          </p>
          <p>
          ...
          <p>
            순금 혜택은 계정당·인증 이메일당 1회 제공되는 혜택이므로, 탈퇴 후
            재가입을 통한 반복 수령을 방지하고 사용하지 않은 적립 순금 잔액을
            같은 인증 이메일의 새 계정으로 승계하기 위해 필요한 최소 범위의
            가명 기록을 서버 전용 영역에 별도 보관합니다. 이 기록에는 원문 이메일,
            성명, 휴대전화번호 또는 회원 UID 원문을 저장하지 않고, 인증된 이메일을
            정규화한 뒤 SHA-256으로 일방향 해시 처리한 식별값, 혜택 종류·수령 여부·
            기록 시각, 미사용 잔액, 승계 상태·시각 및 기존 UID 원문을 복원할 수 없는
            일방향 계정 연속성 식별값을 저장합니다. 보유기간은 회원 유지 기간 및
            회원 탈퇴 후 3년이며, 해당 기록이 보관되는 기간 안에 같은 인증 이메일로
            재가입하면 미사용 잔액을 복원할 수 있습니다. 그 전에 관련 목적이
            소멸하면 지체 없이 삭제합니다.
          </p>
          ...
          <TableWrap>
            <Table>
              ...
              <tbody>
                ...
                <tr>
                  <td>순금 혜택 중복 방지·미사용 잔액 승계 기록</td>
                  <td>
                    인증 이메일의 SHA-256 일방향 해시값, 혜택 종류·수령 여부·
                    기록 시각, 미사용 적립 순금 잔액, 승계 상태·시각과 일방향 계정
                    연속성 식별값을 회원 유지 기간 및 탈퇴 후 3년간 보관.
                    관련 목적이 먼저 소멸하면 지체 없이 삭제
                  </td>
                </tr>
                <tr>
                  <td>접속·보안 로그</td>
