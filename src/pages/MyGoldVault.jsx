...
export default function MyGoldVault() {
  ...
  const guestItems = useMemo(
    () =>
      guestRawItems.map((item) => {
        const pureGoldG = computeVaultPureGoldG(item, rates, customerSellPricePerDon);
        const previousPureGoldG = computeVaultPureGoldG(item, rates, previousCustomerSellPricePerDon);
  ...
  const guestSummary = useMemo(() => {
    ...
    const replacementValueWon = guestItems.reduce(
      (total, item) => total + Number(item.replacementValueWon || 0),
      0
    );
    return {
