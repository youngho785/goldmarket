...
export default function AdminNotifications() {
  ...
  const update = (key) => (event) => {
    ...
    setForm((prev) => {
      if (key === "category") {
        const nextLink =
          value === "goldNews"
            ? (!prev.link.trim() || prev.link === "/" ? "/gold-price" : prev.link)
            : (prev.link === "/gold-price" ? "/" : prev.link);
  ...
  const handleSend = async () => {
    ...
    const isMarketingCategory =
      form.category === "goldNews" || form.category === "benefits";
    ...
    const confirmed = window.confirm(
      `${targetText}에게 알림을 발송하시겠습니까?\n\n${form.title.trim()}\n${form.body.trim()}${
        isMarketingCategory
          ? "\n\n※ 광고성 정보 수신동의가 확인된 회원에게만 발송됩니다."
          : ""
      }`
    );
    if (!confirmed) return;
    ...
    try {
      ...
      setMessage(
        `알림 생성 완료 · 대상 ${Number(result?.recipientCount || 0).toLocaleString("ko-KR")}명`
      );
  ...
  return (
    <Wrap>
      <Card>
        <Title>알림 발송</Title>
        <Intro>
          관리자 알림을 사이트 알림함에 생성합니다. 금시세·주요 소식 및 이벤트·혜택 알림은
          광고성 정보 수신동의가 확인된 회원에게만 발송됩니다.
        </Intro>
          ...
          <Full>
            <Field>
              ...
              <Input
                ...
                maxLength={80}
                placeholder="예: 오늘의 금시세가 업데이트되었습니다"
              />
            </Field>
          </Full>
          ...
          <Full>
            <Field>
              ...
              <Input
                ...
                maxLength={200}
                placeholder="/gold-price 또는 /gold-exchange"
              />
            </Field>
          </Full>
        </Grid>
        ...
        {(form.category === "goldNews" || form.category === "benefits") && (
          <MarketingNotice>
            ...
            서버에서 광고성 정보 수신동의 여부를 다시 확인합니다. 미동의 회원은
            ‘전체’, ‘예약 고객’, ‘특정 사용자’를 선택해도 발송 대상에서 제외됩니다.
          </MarketingNotice>
        )}
        ...
        <Actions>
          <Button type="button" disabled={!canSend || sending} onClick={handleSend}>
            {sending ? "발송 준비 중…" : "알림 발송"}
          </Button>
        </Actions>
      ...
      <Card>
        ...
        <Intro>관리자가 발송한 최근 알림 기록입니다.</Intro>
