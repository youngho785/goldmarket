...
const DEFAULT_PRODUCTS = defaults.products || {};
...
async function main() {
  ...
  for (const [id, product] of Object.entries(DEFAULT_PRODUCTS)) {
    nextProducts[id] = { ...product, ...(nextProducts[id] || {}) };
  }
  ...
  await ref.set(
    {
      ...
      version: Math.max(Number(data.version) || 0, defaults.version),
    },
    { merge: true }
  );
