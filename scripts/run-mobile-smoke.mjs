...
const outputDir = path.resolve(root, "test-results", "mobile-smoke");
...
if (!existsSync(path.join(root, "tests", "smoke", "node_modules", "@playwright", "test"))) {
  ...
  console.log("KGM MOBILE WEB SMOKE");
  console.log("4 Pixel 7 critical UI flows only. Emulator data only. No deployment.");
  ...
  if (build.code !== 0) {
    ...
    const inner = headed
      ? "npm --prefix tests/smoke run test:mobile:inner:headed"
      : "npm --prefix tests/smoke run test:mobile:inner";
    ...
    if (result.code !== 0) {
      fail(`One or more mobile smoke flows failed. Log: ${emulatorLog}`);
      ...
      console.log("\n========================================");
      console.log("MOBILE WEB SMOKE: PASS");
      console.log("4/4 Pixel 7 critical flows completed.");
