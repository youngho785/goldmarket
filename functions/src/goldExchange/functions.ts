...
export const requestGoldExchangeGroup = onCall<{
  ...
  const [ratesSnap, currentPriceSnap] = await Promise.all([
    db().doc("appConfig/goldRates").get(),
    db().doc("goldPrices/current").get(),
  ]);
  const ratesData = ratesSnap.exists ? ratesSnap.data() || {} : {};
  const productsPolicy = mergeGoldProducts(ratesData.products);
  const rates = {
    purity: { ...DEFAULT_PURITY, ...(ratesData.purity || {}) } as Record<string, number>,
    exchange: { ...DEFAULT_EXCHANGE, ...(ratesData.exchange || {}) } as Record<string, number>,
    products: productsPolicy,
    version: Number(ratesData.version) || DEFAULT_GOLD_RATES_VERSION,
  };
  const rateVersion = rates.version;
  ...
  const calculatedProducts = validatedProducts.map((product, index) => {
    const policy = findGoldProductPolicy(productsPolicy, product.productId, product.goldType);
    if (!policy || policy.active === false || policy.exchangeEnabled === false) {
      throw new HttpsError("invalid-argument", `${index + 1}번째 제품은 현재 금교환 대상이 아닙니다.`);
    }
    ...
    if (calculation.requiresManualCheck) {
      throw new HttpsError(
        "failed-precondition",
        `${policy.displayName}은 온라인 자동계산 대상이 아닙니다. 현장 확인 예약으로 진행해 주세요.`
      );
    }
    return {
      ...product,
      ...
      for (let i = 0; i < calculatedProducts.length; i++) {
        ...
        tx.set(
          docRef,
          {
            ...
            calculationMethod: p.calculation.calculationMethod,
            conversionRateUsed: p.calculation.conversionRateUsed,
            refiningFeePerDonUsed: p.calculation.refiningFeePerDonUsed,
            referenceBuyPricePerDonUsed: pureGoldBuyPricePerDon,
            refiningDeductionG: p.calculation.refiningDeductionG,
            exchangeRatioUsed: p.calculation.exchangeRatioUsed,
            // Legacy field retained for older admin/history views.
            purityUsed: p.calculation.conversionRateUsed || 1,
            calcVersion: 6,
            rateVersion,
