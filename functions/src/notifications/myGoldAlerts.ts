...
async function loadUserMetrics(
  ...
  items.docs.forEach((document) => {
    ...
    const recognizedG = computeGoldPolicyResult({
      ...
      products,
      exchange,
      pureGoldBuyPricePerDon: currentPricePerDon,
    }).finalWeightG;
    if (recognizedG <= 0) return;

    registeredItemCount += 1;
    registeredPureGoldG += recognizedG;
    registeredValueWon += goldItemMarketValueWon({
      grams: weightG,
      productId,
      goldType,
      products,
...
function metricsFromAggregate(
  aggregate: VaultAggregate | undefined,
  userData: FirebaseFirestore.DocumentData,
  currentMarket: Record<string, unknown>
): AlertMetrics {
  const currentPricePerDon = Number(currentMarket.pureGoldBuyPerDon) || 0;
  ...
  const registeredValueWon = aggregate?.registeredValueWon || 0;
  const bonusGoldG = roundTo3(readBonusGoldG(userData));
  const exchangeReadyG = roundTo3(registeredPureGoldG + bonusGoldG);
  return {
...
export const saveMyGoldAlertGoals = onCall<{
  ...
  async (request) => {
    ...
    const currentMarket = currentPriceSnap.get("market") || {};
    const currentPricePerDon = Number((currentMarket as Record<string, unknown>).pureGoldBuyPerDon) || 0;
    ...
    const metrics = await loadUserMetrics(
      uid,
      freshUserData,
      currentMarket as Record<string, unknown>,
      rates.products,
      rates.exchange
    );
