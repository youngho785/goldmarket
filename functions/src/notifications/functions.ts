...
async function loadWeeklyGoldReferenceMarket(
  ...
  if (exact.exists) {
    const market = (exact.get("market") || {}) as Record<string, unknown>;
    if ((Number(market.pureGoldBuyPerDon) || 0) > 0) return market;
  }
  ...
  return fallbackDoc
    ? (fallbackDoc.get("market") || {}) as Record<string, unknown>
    : {};
}
...
export const sendMyGoldWeeklyReports = onSchedule(
  ...
  async () => {
    ...
    const currentMarket = (currentPriceSnap.get("market") || {}) as Record<string, unknown>;
    const currentPrice = Number(currentMarket.pureGoldBuyPerDon) || 0;
    if (currentPrice <= 0) {
      console.log(`[sendMyGoldWeeklyReports] date=${today} skipped=current-price-missing`);
      return;
    }

    const historicalMarket = await loadWeeklyGoldReferenceMarket(referenceDate);
    const historicalPrice = Number(historicalMarket.pureGoldBuyPerDon) || 0;
    const ratesData = ratesSnap.exists ? ratesSnap.data() || {} : {};
    const products = mergeGoldProducts(ratesData.products) as typeof DEFAULT_GOLD_PRODUCTS;
    const exchange = {
      ...DEFAULT_EXCHANGE,
      ...(ratesData.exchange && typeof ratesData.exchange === "object" ? ratesData.exchange : {}),
    } as Record<string, number>;

    const vaultByUid = new Map<string, {
      itemCount: number;
      pureGoldG: number;
      currentValueWon: number;
      historicalValueWon: number;
    }>();
    vaultSnapshot.docs.forEach((document) => {
      ...
      const recognizedG = computeGoldPolicyResult({
        grams: weightG,
        productId,
        goldType,
        ...
        products,
        exchange,
        pureGoldBuyPricePerDon: currentPrice,
      }).finalWeightG;
      if (recognizedG <= 0) return;

      const current = vaultByUid.get(uid) || {
        itemCount: 0,
        pureGoldG: 0,
        currentValueWon: 0,
      ...
      current.currentValueWon += goldItemMarketValueWon({
        grams: weightG,
        productId,
        goldType,
        products,
      ...
      if (historicalPrice > 0) {
        current.historicalValueWon += goldItemMarketValueWon({
          grams: weightG,
          productId,
          goldType,
          products,
    ...
    for (const userDocument of usersSnapshot.docs) {
      ...
      const vault = vaultByUid.get(uid) || {
        itemCount: 0,
        pureGoldG: 0,
        currentValueWon: 0,
        historicalValueWon: 0,
