...
export type GoldCalculationMethod = "rate" | "refining_fee" | "full" | "manual";
export type GoldValuationPriceBasis = "pure" | "18k" | "14k";
export type GoldProductPolicy = {
  id: string;
  displayName: string;
  legacyGoldType: string;
  calculationMethod: GoldCalculationMethod;
  conversionRate: number;
  refiningFeePerDon: number;
  valuationPriceBasis: GoldValuationPriceBasis;
  active: boolean;
  myGoldEnabled: boolean;
  exchangeEnabled: boolean;
  sortOrder: number;
};
export const DEFAULT_GOLD_PRODUCTS: Record<string, GoldProductPolicy> = Object.fromEntries(
  Object.entries(goldRatesDefaults.products || {}).map(([id, raw]) => [
    id,
    { id, ...(raw as Omit<GoldProductPolicy, "id">) },
  ])
);
...
export function validateProducts(value: unknown): ValidatedProduct[] {
  ...
  const validated = value.map((raw, index) => {
    ...
    const productId = String(product.productId || "").trim().slice(0, 80);
    const goldType = String(product.goldType || "").trim().slice(0, 180);
    ...
    if (!productId && !goldType) {
      throw new HttpsError("invalid-argument", `${index + 1}번째 제품 종류를 선택해 주세요.`);
    }
    ...
    return { productId, goldType, quantity, inputUnit, exchangeType, grams };
  });
...
const LEGACY_DEFAULT_DISPLAY_NAMES: Readonly<Record<string, string>> = Object.freeze({
...
function normalizeGoldProductDisplayName(
  id: string,
  rawName: unknown,
...
export function normalizeGoldProductPolicy(
  id: string,
  raw: unknown,
  fallback?: GoldProductPolicy
): GoldProductPolicy {
  const source = raw && typeof raw === "object" && !Array.isArray(raw)
    ? raw as Record<string, unknown>
    : {};
  const base = fallback || ({} as GoldProductPolicy);
  const methodRaw = String(source.calculationMethod ?? base.calculationMethod ?? "manual");
  const calculationMethod: GoldCalculationMethod =
    methodRaw === "rate" || methodRaw === "refining_fee" || methodRaw === "full"
      ? methodRaw
      : "manual";
  const conversionRateRaw = Number(source.conversionRate ?? base.conversionRate ?? 0);
  const refiningFeeRaw = Number(source.refiningFeePerDon ?? base.refiningFeePerDon ?? 0);
  const sortOrderRaw = Number(source.sortOrder ?? base.sortOrder ?? 999);
  const basisRaw = String(source.valuationPriceBasis ?? base.valuationPriceBasis ?? "").toLowerCase();
  const inferText = `${id} ${String(source.displayName ?? base.displayName ?? "")} ${String(source.legacyGoldType ?? base.legacyGoldType ?? "")}`;
  const valuationPriceBasis: GoldValuationPriceBasis = basisRaw === "14k" || basisRaw === "18k" || basisRaw === "pure"
    ? basisRaw
    : /14k/i.test(inferText)
      ? "14k"
      : /18k/i.test(inferText)
        ? "18k"
        : "pure";
  return {
    id,
    ...
    calculationMethod,
    conversionRate: Number.isFinite(conversionRateRaw)
      ? Math.max(0, Math.min(1, conversionRateRaw))
      : 0,
    refiningFeePerDon: Number.isFinite(refiningFeeRaw)
      ? Math.max(0, Math.round(refiningFeeRaw))
      : 0,
    valuationPriceBasis,
    active: source.active !== undefined ? source.active === true : base.active !== false,
    myGoldEnabled: source.myGoldEnabled !== undefined ? source.myGoldEnabled === true : base.myGoldEnabled !== false,
    exchangeEnabled: source.exchangeEnabled !== undefined ? source.exchangeEnabled === true : base.exchangeEnabled !== false,
    sortOrder: Number.isFinite(sortOrderRaw) ? Math.round(sortOrderRaw) : 999,
  };
}

export function mergeGoldProducts(raw: unknown): Record<string, GoldProductPolicy> {
  const remote = raw && typeof raw === "object" && !Array.isArray(raw)
    ? raw as Record<string, unknown>
    : {};
  const ids = new Set([...Object.keys(DEFAULT_GOLD_PRODUCTS), ...Object.keys(remote)]);
  return Object.fromEntries(
    [...ids].map((id) => [id, normalizeGoldProductPolicy(id, remote[id], DEFAULT_GOLD_PRODUCTS[id])])
  );
}

export function findGoldProductPolicy(
  products: Record<string, GoldProductPolicy>,
  productId?: string,
  goldType?: string
): GoldProductPolicy | null {
...
export function goldProductBuyPricePerDon(
  product: GoldProductPolicy | null,
  market: Record<string, unknown> | undefined
): number {
  const basis = product?.valuationPriceBasis || "pure";
  const key = basis === "14k"
    ? "gold14kBuyPerDon"
    : basis === "18k"
      ? "gold18kBuyPerDon"
...
export function goldItemMarketValueWon(params: {
  grams: number;
  productId?: string;
  goldType?: string;
  products?: Record<string, GoldProductPolicy>;
  ...
  const products = params.products || DEFAULT_GOLD_PRODUCTS;
  const product = findGoldProductPolicy(products, params.productId, params.goldType);
  const pricePerDon = goldProductBuyPricePerDon(product, params.market);
  if (!Number.isFinite(grams) || grams <= 0 || pricePerDon <= 0) return 0;

  const inputDon = grams / DON_TO_GRAMS;
  const grossValueWon = inputDon * pricePerDon;
...
export function computeGoldPolicyResult(params: {
  grams: number;
  productId?: string;
  goldType?: string;
  exchangeType?: string;
  ...
  pureGoldBuyPricePerDon?: number;
}): {
  finalWeightG: number;
  product: GoldProductPolicy | null;
  calculationMethod: GoldCalculationMethod;
  conversionRateUsed: number;
  refiningFeePerDonUsed: number;
  refiningFeeWon: number;
  refiningDeductionG: number;
  exchangeRatioUsed: number;
  requiresManualCheck: boolean;
} {
  const products = params.products || DEFAULT_GOLD_PRODUCTS;
  const product = findGoldProductPolicy(products, params.productId, params.goldType);
  const grams = Number(params.grams);
  const exchangeRatioRaw = Number(
    params.exchange?.[params.exchangeType || ""] ?? DEFAULT_EXCHANGE[params.exchangeType || ""] ?? 1
  );
  const exchangeRatioUsed = Number.isFinite(exchangeRatioRaw) ? exchangeRatioRaw : 1;
  const base = {
    finalWeightG: 0,
    product,
    calculationMethod: product?.calculationMethod || "manual" as GoldCalculationMethod,
  ...
  if (!product || !Number.isFinite(grams) || grams <= 0) return base;
  if (product.calculationMethod === "manual") return base;
  if (product.calculationMethod === "rate") {
    const conversionRateUsed = Number(product.conversionRate) || 0;
    return {
      ...base,
      finalWeightG: roundTo3(grams * conversionRateUsed * exchangeRatioUsed),
      conversionRateUsed,
  ...
  if (product.calculationMethod === "full") {
    return {
      ...base,
      finalWeightG: roundTo3(grams * exchangeRatioUsed),
      conversionRateUsed: 1,
      requiresManualCheck: false,
    };
  }
  const pricePerDon = Number(params.pureGoldBuyPricePerDon);
  const refiningFeePerDonUsed = Math.max(0, Number(product.refiningFeePerDon) || 0);
  if (!Number.isFinite(pricePerDon) || pricePerDon <= 0) {
    return { ...base, refiningFeePerDonUsed };
  }
  const refiningFeeWon = Math.round((grams / DON_TO_GRAMS) * refiningFeePerDonUsed);
  const refiningDeductionG = roundTo3((refiningFeeWon / pricePerDon) * DON_TO_GRAMS);
  return {
    ...base,
    finalWeightG: roundTo3(Math.max(0, grams - refiningDeductionG) * exchangeRatioUsed),
    conversionRateUsed: 1,
    refiningFeePerDonUsed,
    refiningFeeWon,
    refiningDeductionG,
    requiresManualCheck: false,
  };
...
// Legacy helper retained for historical code paths. New exchange/MY GOLD logic uses computeGoldPolicyResult.
