...
function normalizeGoldProductTable(value: unknown): Record<string, GoldProductPolicy> {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new HttpsError("invalid-argument", "금제품 정책 형식이 올바르지 않습니다.");
  }
  const raw = value as Record<string, unknown>;
  const ids = Object.keys(raw);
  if (ids.length < 1 || ids.length > 80) {
    throw new HttpsError("invalid-argument", "금제품은 1개 이상 80개 이하로 관리할 수 있습니다.");
  }
  return Object.fromEntries(ids.map((id) => {
    if (!/^[a-z0-9][a-z0-9-]{2,79}$/.test(id)) {
      throw new HttpsError("invalid-argument", `제품 ID 형식이 올바르지 않습니다: ${id}`);
    }
    const source = raw[id];
    if (!source || typeof source !== "object" || Array.isArray(source)) {
      throw new HttpsError("invalid-argument", `${id} 제품 설정이 올바르지 않습니다.`);
    }
    const row = source as Record<string, unknown>;
    const displayName = String(row.displayName || "").trim();
    const legacyGoldType = String(row.legacyGoldType || "").trim();
    const calculationMethod = String(row.calculationMethod || "");
    const conversionRate = Number(row.conversionRate);
    const refiningFeePerDon = Number(row.refiningFeePerDon);
    const valuationPriceBasis = String(row.valuationPriceBasis || "pure").toLowerCase();
    const sortOrder = Math.trunc(Number(row.sortOrder));
    if (displayName.length < 1 || displayName.length > 80) {
      throw new HttpsError("invalid-argument", `${id} 제품명을 확인해 주세요.`);
    }
    if (legacyGoldType.length > 180) {
      throw new HttpsError("invalid-argument", `${id} 기존 제품 식별값이 너무 깁니다.`);
    }
    if (!["rate", "refining_fee", "full", "manual"].includes(calculationMethod)) {
      throw new HttpsError("invalid-argument", `${displayName} 계산방식을 확인해 주세요.`);
    }
    if (calculationMethod === "rate" && (!Number.isFinite(conversionRate) || conversionRate <= 0 || conversionRate > 1)) {
      throw new HttpsError("invalid-argument", `${displayName} 환산율은 0보다 크고 100% 이하여야 합니다.`);
    }
    if (calculationMethod === "refining_fee" && (!Number.isFinite(refiningFeePerDon) || refiningFeePerDon < 0 || refiningFeePerDon > 1_000_000)) {
      throw new HttpsError("invalid-argument", `${displayName} 정련비를 확인해 주세요.`);
    }
    if (!["pure", "18k", "14k"].includes(valuationPriceBasis)) {
      throw new HttpsError("invalid-argument", `${displayName} MY GOLD 가치 기준을 확인해 주세요.`);
    }
    if (!Number.isInteger(sortOrder) || sortOrder < 0 || sortOrder > 10000) {
      throw new HttpsError("invalid-argument", `${displayName} 표시순서를 확인해 주세요.`);
    }
    return [id, {
      id,
      displayName,
      legacyGoldType,
      calculationMethod: calculationMethod as GoldProductPolicy["calculationMethod"],
      conversionRate: calculationMethod === "rate" ? Math.round(conversionRate * 100_000) / 100_000 : calculationMethod === "full" || calculationMethod === "refining_fee" ? 1 : 0,
      refiningFeePerDon: calculationMethod === "refining_fee" ? Math.round(refiningFeePerDon) : 0,
      valuationPriceBasis: valuationPriceBasis as GoldProductPolicy["valuationPriceBasis"],
      active: row.active === true,
      myGoldEnabled: calculationMethod === "manual" ? false : row.myGoldEnabled === true,
      exchangeEnabled: row.exchangeEnabled === true,
      sortOrder,
    } satisfies GoldProductPolicy];
  }));
}
...
function buildLegacyPurityTable(
  ...
  Object.values(products).forEach((product) => {
    ...
    if (product.calculationMethod === "rate") result[product.legacyGoldType] = product.conversionRate;
    if (product.calculationMethod === "full") result[product.legacyGoldType] = 1;
  });
  return result;
}
...
export const updateGoldRates = onCall<{
  ...
  async (req) => {
    ...
    const products = normalizeGoldProductTable(req.data?.products);
    const exchange = req.data?.exchange
      ? normalizeRateTable(req.data.exchange, DEFAULT_EXCHANGE, "교환율")
      : { ...DEFAULT_EXCHANGE };
    ...
    const nextVersion = await db().runTransaction(async (tx) => {
      ...
      const purity = buildLegacyPurityTable(products, before.purity);
      tx.set(ratesRef, {
        products,
        purity,
        exchange,
      ...
      tx.set(historyRef, {
        ...
        actorUid,
        before: {
          products: { ...DEFAULT_GOLD_PRODUCTS, ...(before.products || {}) },
          purity: { ...DEFAULT_PURITY, ...(before.purity || {}) },
          exchange: { ...DEFAULT_EXCHANGE, ...(before.exchange || {}) },
          version: currentVersion,
        },
        after: { products, purity, exchange, version },
        createdAt: updatedAt,
      });
      return version;
    });
