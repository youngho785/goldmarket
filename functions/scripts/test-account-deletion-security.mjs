...
test("계정삭제 호출 순서·필수 경로·로컬/CI 명령의 회귀를 방지한다", async () => {
  const [
    ...
    readFile(new URL("../src/account/functions.ts", import.meta.url), "utf8"),
  ...
  // index.ts가 account/functions.ts의 계정 함수를 정상적으로 재-export하는지 확인
  assert.match(
    indexSource,
    /export\s*\{[\s\S]*?deleteMyAccount[\s\S]*?submitGoldExchangeReview[\s\S]*?\}\s*from\s*["']\.\/account\/functions\.js["']/
  );

  // 실제 계정삭제 구현은 account/functions.ts에서 검사
  const deleteStart = accountSource.indexOf("export const deleteMyAccount");
  const deleteEnd = accountSource.indexOf(
    "export const submitGoldExchangeReview",
    deleteStart
  );

  assert.ok(deleteStart >= 0 && deleteEnd > deleteStart);
  const deleteBlock = accountSource.slice(deleteStart, deleteEnd);
