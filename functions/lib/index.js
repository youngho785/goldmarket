...
function computeFinalWeightFromRates(params) {
    ...
    const p = typeof purity?.[goldType ?? ""] === "number"
        ? purity[goldType]
        : (DEFAULT_PURITY[goldType ?? ""] ?? 0);
    const e = typeof exchange?.[exchangeType ?? ""] === "number"
        ? exchange[exchangeType]
        : (DEFAULT_EXCHANGE[exchangeType ?? ""] ?? 1);
    return roundTo3(grams * p * e);
}
async function addNotificationForUser(uid, payload) {
    if (!uid)
        return;
 ...
 * ───────────────────────────────────────────────────────────── */
export const onChatMessageCreate = onDocumentCreated({ region: "asia-northeast3", document: "chats/{chatId}/messages/{messageId}" }, async (event) => {
    try {
        ...
        // (A) 채팅 메타 업데이트
        const lastMessage = (message.imageUrl && "[이미지]") ||
        ...
        await db().runTransaction(async (tx) => {
            ...
            receivers.forEach((uid) => {
                ...
                tx.set(metaRef, { unreadTotal: FieldValue.increment(1) }, { merge: true });
            });
        });
        // (B) 푸시 발송 — 에뮬레이터에서는 스킵
        if (!receivers.length || IN_EMULATOR)
            return;
        // 수신자들의 FCM 토큰 수집 (중복 제거)
        const tokenSet = new Set();
        await Promise.all(receivers.map(async (uid) => {
            const uSnap = await db().doc(`users/${uid}`).get();
            const tokens = (uSnap.get("fcmTokens") || []);
            (tokens || []).forEach((t) => {
        ...
        const body = message.imageUrl ? "이미지를 보냈습니다." : message.text || "새 메시지가 도착했습니다.";
        ...
        if (bad.length) {
            await Promise.all(receivers.map((uid) => db().doc(`users/${uid}`).update({ fcmTokens: FieldValue.arrayRemove(...bad) })));
        }
    }
    catch (err) {
...
export const markChatAsRead = onCall({ region: "asia-northeast3" }, async (req) => {
    ...
    await db().runTransaction(async (tx) => {
        ...
        const participantsMap = chatData.participantsMap && typeof chatData.participantsMap === "object"
        ...
        // 2) 전체 미읽음 합계 차감
        if (prevUnread > 0) {
            tx.set(metaRef, { unreadTotal: Math.max(0, curTotal - prevUnread) }, { merge: true });
        }
 ...
 * 2) 예약 슬롯 해제 (관리자 UI용)
 * ───────────────────────────────────────────────────────────── */
export const releaseReservedSlot = onCall({ region: "asia-northeast3" }, async (req) => {
    requireAdmin((req.auth?.token || {}));
    const { dateKey, time } = (req.data || {});
    if (!dateKey ||
        !time ||
    ...
    await db().runTransaction(async (tx) => {
        ...
        if (Array.isArray(raw?.[dateKey])) {
            const next = (raw[dateKey] || []).filter((t) => t !== time);
            tx.set(ref, { [dateKey]: next }, { merge: true });
        }
        else {
            tx.set(ref, { [`${dateKey}.${time}`]: FieldValue.delete() }, { merge: true });
        }
    });
 ...
 * 관리자 역할 변경 (최고 관리자 전용)
 * ───────────────────────────────────────────────────────────── */
export const setUserRole = onCall({ region: "asia-northeast3" }, async (req) => {
    const callerUid = req.auth?.uid;
    if (!callerUid)
        throw new HttpsError("unauthenticated", "로그인이 필요합니다.");
    requireSuperAdmin((req.auth?.token || {}));
    ...
    await db().doc(`users/${uid}`).set({ role, roleUpdatedAt: FieldValue.serverTimestamp(), roleUpdatedBy: callerUid }, { merge: true });
    return { ok: true, uid, role };
});
/* ─────────────────────────────────────────────────────────────
 * 3) 그룹 생성 + 슬롯 선점 (사용자 제출)
 * ───────────────────────────────────────────────────────────── */
export const requestGoldExchangeGroup = onCall({ region: "asia-northeast3" }, async (req) => {
    const uid = req.auth?.uid;
    if (!uid)
        throw new HttpsError("unauthenticated", "로그인이 필요합니다.");
    const { visitDate, visitTime, name, phone, email = null, privacyConsent, privacyConsentVersion, products = [], barsPlan = null, } = (req.data || {});
    ...
    await db().runTransaction(async (tx) => {
        ...
        const taken = Array.isArray(v)
            ? v.includes(visitTime)
            : !!(v && typeof v === "object" && v[visitTime]);
        if (taken)
            throw new HttpsError("aborted", "이미 예약된 시간입니다.");
        // 2) 슬롯 선점
        tx.set(slotsRef, { [visitDate]: { [visitTime]: true } }, { merge: true });
        // 3) 문서 생성
        if (Array.isArray(products) && products.length > 0) {
            for (let i = 0; i < products.length; i++) {
                const p = products[i] || {};
                const qty = Number(p.quantity || 0);
                const unit = p.inputUnit === "don" ? "don" : "g";
 ...
 * 4) 그룹 상태 일괄 변경 (관리자)
 * ───────────────────────────────────────────────────────────── */
export const setExchangeGroupStatus = onCall({ region: "asia-northeast3" }, async (req) => {
    const claims = (req.auth?.token || {});
    if (!(claims.admin === true || claims.superAdmin === true)) {
        throw new HttpsError("permission-denied", "관리자 권한이 필요합니다.");
    }
    const { groupId, status } = (req.data || {});
    if (!groupId || !status)
        throw new HttpsError("invalid-argument", "groupId와 status가 필요합니다.");
    const now = FieldValue.serverTimestamp();
    const extra = {};
    if (status === "scheduled")
    ...
    if (status === "completed" && bonusUsageStatus === "requested") {
        throw new HttpsError("failed-precondition", "적립 순금 사용 신청을 먼저 확정하거나 취소해 주세요.");
    }
    const batch = db().batch();
    let targetUid = null;
    let targetVisitDate = "";
    ...
    qs.docs.forEach((d) => {
        ...
        batch.update(d.ref, { status, updatedAt: now, ...extra });
    });
    await batch.commit();
    if (status === "canceled" ||
        status === "rejected" ||
 ...
 * 5) 그룹 요약 집계
 * ───────────────────────────────────────────────────────────── */
export const aggregateGoldExchangeGroup = onDocumentWritten({ region: "asia-northeast3", document: "goldExchanges/{docId}" }, async (event) => {
    ...
    qs.docs.forEach((d) => {
        ...
        if (idx > -1 && (ridx === -1 || idx < ridx)) {
            repStatus = (x.status || "requested");
        }
        const c = x.createdAt instanceof Date
            ? x.createdAt
            : x.createdAt?.toDate?.() ?? null;
        const u = x.updatedAt instanceof Date
            ? x.updatedAt
            : x.updatedAt?.toDate?.() ?? null;
        if (!createdAt || (c && c < createdAt))
            createdAt = c;
        if (!updatedAt || (u && u > updatedAt))
            updatedAt = u;
        if (!ownerUid && x.userId)
            ownerUid = x.userId;
        if (!visitDate && x.visitDate)
            visitDate = x.visitDate;
        if (!visitTime && x.visitTime)
 ...
 * 6) 알림 문서 생성 시 FCM 발송
 * ───────────────────────────────────────────────────────────── */
export const onNotificationCreate = onDocumentCreated({ region: "asia-northeast3", document: "notifications/{uid}/items/{docId}" }, async (event) => {
    try {
        if (IN_EMULATOR)
            return;
        ...
        const tokens = (userSnap.get("fcmTokens") || []).filter((t) => typeof t === "string" && t.length > 0);
        if (!tokens.length)
            return;
 ...
 * 7) 예약 슬롯 청소 (스케줄러)
 * ───────────────────────────────────────────────────────────── */
export const cleanReservedSlots = onSchedule({ schedule: "every 60 minutes", timeZone: "Asia/Seoul", region: "asia-northeast3" }, async () => {
    const toYmdSeoul = () => {
 ...
 * 8) 닉네임 유니크 인덱스 (콜러블)
 * ───────────────────────────────────────────────────────────── */
const normalizeNickname = (raw) => {
    const original = String(raw || "").trim();
    const lower = original.toLocaleLowerCase();
    return { lower, original };
};
/** 회원가입 직후 닉네임 선점 */
export const claimNickname = onCall({ region: "asia-northeast3" }, async (req) => {
    const uid = req.auth?.uid;
    if (!uid)
        throw new HttpsError("unauthenticated", "로그인이 필요합니다.");
    const raw = (req.data?.nickname || "").trim();
    if (!raw)
        throw new HttpsError("invalid-argument", "nickname이 필요합니다.");
    if (raw.length > 16)
        throw new HttpsError("invalid-argument", "닉네임은 16자 이내여야 합니다.");
...
/** 닉네임 변경 */
export const changeNickname = onCall({ region: "asia-northeast3" }, async (req) => {
    const uid = req.auth?.uid;
    if (!uid)
        throw new HttpsError("unauthenticated", "로그인이 필요합니다.");
    const raw = (req.data?.newNickname || "").trim();
    if (!raw)
        throw new HttpsError("invalid-argument", "newNickname이 필요합니다.");
    if (raw.length > 16)
        throw new HttpsError("invalid-argument", "닉네임은 16자 이내여야 합니다.");
    ...
    await db().runTransaction(async (tx) => {
        ...
        const prevLower = (pSnap.exists ? pSnap.get("nicknameLower") : undefined) || undefined;
        ...
        // 점유 가능 검사
        if (newSnap.exists) {
            const owner = (newSnap.data() || {}).ownerUid;
            if (owner && owner !== uid) {
                throw new HttpsError("already-exists", "이미 사용 중인 닉네임입니다.");
            }
        }
        ...
        if (prevLower && prevLower !== newLower && prevRef && prevSnap?.exists) {
            const prevOwner = (prevSnap.data() || {}).ownerUid;
            if (prevOwner === uid) {
                tx.delete(prevRef);
            }
        }
 ...
 * ───────────────────────────────────────────────────────────── */
async function deleteCollectionInBatches(colRef, whereField, whereEq, batchSize = 250) {
    let last = null;
    for (;;) {
        let q = colRef.orderBy("__name__").limit(batchSize);
        if (whereField)
            q = q.where(whereField, "==", whereEq);
        if (last)
            q = q.startAfter(last);
...
export const deleteMyAccount = onCall({ region: "asia-northeast3", timeoutSeconds: 540 }, async (req) => {
    const uid = req.auth?.uid;
    if (!uid)
        throw new HttpsError("unauthenticated", "로그인이 필요합니다.");
    ...
    const prevLower = profSnap.exists ? profSnap.get("nicknameLower") : null;
    await db().runTransaction(async (tx) => {
        // ---- READ FIRST (조건부) ----
        let nickRef = null;
        let nSnap = null;
        ...
        if (prevLower && nickRef && nSnap?.exists) {
            const owner = (nSnap.data() || {}).ownerUid;
            if (owner === uid)
                tx.delete(nickRef);
        }
    });
 ...
 * ───────────────────────────────────────────────────────────── */
export const submitTransactionReview = onCall({ region: "asia-northeast3" }, async (req) => {
    const uid = req.auth?.uid;
    if (!uid)
        throw new HttpsError("unauthenticated", "로그인이 필요합니다.");
 ...
 * ───────────────────────────────────────────────────────────── */
export const submitGoldExchangeReview = onCall({ region: "asia-northeast3" }, async (req) => {
    const uid = req.auth?.uid;
    if (!uid)
        throw new HttpsError("unauthenticated", "로그인이 필요합니다.");
    ...
    await db().runTransaction(async (tx) => {
        ...
        if (completedExchange.userId !== uid ||
        ...
        for (const exchangeDoc of ownedDocs) {
            tx.set(exchangeDoc.ref, { reviewed: true, reviewedAt: now }, { merge: true });
        }
    });
...
function bonusBalanceMilliGrams(data) {
...
async function resolveWelcomeBonusState(uid) {
    const userRef = db().doc(`users/${uid}`);
    ...
    return db().runTransaction(async (tx) => {
        ...
        if (promoSnap.exists) {
            ...
            const creditedMg = toNonNegativeInteger(promo.creditedMilliGrams, Math.round(Number(promo.creditedG || WELCOME_BONUS_CREDIT_G) * 1000));
...
async function resolveQuizBonusState(uid, claim) {
    const userRef = db().doc(`users/${uid}`);
    ...
    return db().runTransaction(async (tx) => {
        ...
        if (promoSnap.exists) {
            const promo = promoSnap.data() || {};
            const creditedMg = toNonNegativeInteger(promo.creditedMilliGrams, Math.round(Number(promo.creditedG || QUIZ_BONUS_CREDIT_G) * 1000));
...
export const welcomeClaimGoldBonus = onCall({ region: "asia-northeast3" }, async (req) => {
    const uid = req.auth?.uid;
    if (!uid)
        throw new HttpsError("unauthenticated", "로그인이 필요합니다.");
...
export const quizGetGoldBonusStatus = onCall({ region: "asia-northeast3" }, async (req) => {
    const uid = req.auth?.uid;
    if (!uid)
        throw new HttpsError("unauthenticated", "로그인이 필요합니다.");
    return resolveQuizBonusState(uid);
});
export const quizClaimGoldBonus = onCall({ region: "asia-northeast3" }, async (req) => {
    const uid = req.auth?.uid;
    if (!uid)
        throw new HttpsError("unauthenticated", "로그인이 필요합니다.");
    const answerKey = { q1: 0, q2: 0, q3: 1, q4: 0, q5: 0 };
    ...
    if (Object.keys(answers).length !== answerIds.length ||
        !answerIds.every((id) => Number.isInteger(Number(answers[id])))) {
        throw new HttpsError("invalid-argument", "모든 퀴즈 답안을 제출해 주세요.");
    }
    const score = answerIds.reduce((total, id) => total + (Number(answers[id]) === answerKey[id] ? 1 : 0), 0);
    const attemptId = req.data?.attemptId ? String(req.data.attemptId).slice(0, 64) : "";
...
function cleanUsageCode(value) {
    return String(value || "").replace(/\D/g, "").slice(0, 6);
}
function requestCreatedMillis(value) {
...
function publicBonusUsageRequest(data) {
    if (!data?.status)
        return null;
...
export const bonusGetGoldUsageState = onCall({ region: "asia-northeast3" }, async (req) => {
    const uid = req.auth?.uid;
    if (!uid)
        throw new HttpsError("unauthenticated", "로그인이 필요합니다.");
    const userRef = db().doc(`users/${uid}`);
...
export const bonusRequestGoldUsage = onCall({ region: "asia-northeast3" }, async (req) => {
    const uid = req.auth?.uid;
    if (!uid)
        throw new HttpsError("unauthenticated", "로그인이 필요합니다.");
...
export const bonusCancelGoldUsage = onCall({ region: "asia-northeast3" }, async (req) => {
    const uid = req.auth?.uid;
    if (!uid)
        throw new HttpsError("unauthenticated", "로그인이 필요합니다.");
    const requestRef = db().doc(`bonusGoldRedemptionRequests/${uid}`);
...
export const bonusAdminConfirmGoldUsage = onCall({ region: "asia-northeast3" }, async (req) => {
    requireAdmin((req.auth?.token || {}));
    ...
    let exchangeDocs = (await exchangeQuery.get()).docs;
    if (exchangeDocs.length === 0) {
    ...
    const result = await db().runTransaction(async (tx) => {
        const groupSnap = await tx.get(groupRef);
        if (!groupSnap.exists)
            throw new HttpsError("not-found", "금교환 예약 요약을 찾을 수 없습니다.");
...
export const bonusAdminCancelGoldUsage = onCall({ region: "asia-northeast3" }, async (req) => {
    requireAdmin((req.auth?.token || {}));
    const groupId = cleanGroupId(req.data?.groupId);
    const reason = String(req.data?.reason || "매장 확인 중 신청 취소").trim().slice(0, 200);
    if (!groupId)
        throw new HttpsError("invalid-argument", "교환번호가 필요합니다.");
    const groupRef = db().doc(`goldExchangeGroups/${groupId}`);
    const result = await db().runTransaction(async (tx) => {
        const groupSnap = await tx.get(groupRef);
